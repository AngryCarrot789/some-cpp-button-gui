...
{
public: 
	someWindow(Application* app, 
		LPCWSTR title = L"Window Here",
		int x = 100, int y = 100, 
		int nWidth = 1280, int nHeight = 720) : Window(app, title, x, y, nWidth, nHeight)
	{
...
#define thingy(x) LOWORD(x) 
    ...
    LRESULT CALLBACK CustomWndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) override
    {
        ...
        std::cout << eventCounter << " | " << (translatedMessage) << 
            " -> WPARAM " << (wParam) << " -> LPARAM " << (wParam) <<
